/**
 * Migracion aditiva: WaMessage.buttons (titulos de los botones del mensaje,
 * para pintarlos en el hilo igual que en el celular). ADD COLUMN hereda permisos.
 * Correr desde apps/api: node --env-file=.env.local scripts/migrate-wa-buttons.mjs
 */
import pg from 'pg';
...
async function main() {
  ...
  for (const t of tenants) {
    ...
    try {
      await db.connect();
      await db.query(`ALTER TABLE "WaMessage" ADD COLUMN IF NOT EXISTS "buttons" JSONB`);
      console.log(`  ✓ ${t.slug}`);
    } catch (err) {
