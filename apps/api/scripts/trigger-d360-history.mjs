/**
 * Dispara la sincronizacion del HISTORIAL de coexistencia (SMB App Data API):
 * Meta manda hasta 6 meses de chats del celular al webhook (en fases/chunks).
 * ¡OJO! Solo se puede pedir UNA vez por ciclo de onboarding — correr SOLO con
 * el handler de `history` ya DESPLEGADO en Railway.
 *
 * Correr desde apps/api:  node --env-file=.env.local scripts/trigger-d360-history.mjs
 */
import pg from 'pg';
...
async function main() {
  ...
  for (const syncType of ['smb_app_state', 'history']) {
    for (const path of ['/smb_app_data', '/v1/smb_app_data']) {
      console.log(`\nPOST ${path}  sync_type=${syncType} ...`);
      try {
        const res = (await http.post(path, { messaging_product: 'whatsapp', sync_type: syncType })).data;
        console.log('OK:', JSON.stringify(res, null, 2).slice(0, 600));
        break; // este sync_type quedo pedido; no repetir por el otro path
      } catch (e) {
        console.log('error:', e.response?.status, JSON.stringify(e.response?.data ?? e.message).slice(0, 600));
      }
    }
  }
  console.log('\nListo. El historial llega por el webhook en los proximos minutos (fases/chunks).');
}
