/**
 * Migracion aditiva: WaContact.flowState + draftAddress (estado del flujo de
 * confirmacion nativo por Cloud API). ADD COLUMN hereda permisos.
 * Correr desde apps/api: node --env-file=.env.local scripts/migrate-wa-flowstate.mjs
 */
import pg from 'pg';
...
async function main() {
  ...
  for (const t of tenants) {
    ...
    try {
      await db.connect();
      await db.query(`ALTER TABLE "WaContact" ADD COLUMN IF NOT EXISTS "flowState" TEXT`);
      await db.query(`ALTER TABLE "WaContact" ADD COLUMN IF NOT EXISTS "draftAddress" TEXT`);
      console.log(`  ✓ ${t.slug}`);
    } catch (err) {
