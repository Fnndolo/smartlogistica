/**
 * Crea la plantilla GENERAL de saludo (sin variables): sirve para retomar
 * cualquier conversacion fuera de la ventana de 24h desde el picker "/".
 * Correr desde apps/api:  node --env-file=.env.local scripts/create-d360-template-saludo.mjs
 */
import pg from 'pg';
...
async function main() {
  ...
  console.log(`Creando plantilla ${TEMPLATE.name} (es, ${TEMPLATE.category})...`);
  try {
    const res = (await http.post('/v1/configs/templates', TEMPLATE)).data;
    console.log(`-> ${res.name} [${res.language}] categoria=${res.category} estado=${res.status}`);
  } catch (e) {
    console.log('error crear:', e.response?.status, JSON.stringify(e.response?.data ?? e.message, null, 2).slice(0, 900));
  }
}
